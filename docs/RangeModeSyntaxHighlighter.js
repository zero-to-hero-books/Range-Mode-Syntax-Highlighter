...
function RMSHStyler(HTMLelement, lineNumbers){
	...
	function applyStyle(offset, length, styles){
		...
		var el;
		if(offset > charCnt){
			text=document.createTextNode(str.substring(charCnt, offset));
	...
	function updateView(totalLineCnt){
		...
		if(foldsToDisplay.length>0 || lineNumbers){
			...
			var linePad="";
			if(lineNumbers){
				lineGutterWidth=(''+totalLineCnt).length;
				gutterLine=document.createElement('span');
				gutterLine.className="RMSHgutterLine";
				gutterLine.style.cssText='left:0;top:0;position:absolute;height:100%;text-align:right;overflow:hidden;';
			...
			if(foldsToDisplay.length>0){
				...
				gutterFold.style.cssText='left:'+lineGutterWidth+'em;top:0;position:absolute;height:100%;overflow:hidden;text-align:center;';
			...
			blockEl.style.paddingLeft=lineGutterWidth+'em';
			blockEl.appendChild(gutterFrag);
		}
	...
	function addToFold(node, foldNode, fold, offset){
		 //we have a non null nextNode for sure, we can do a do/while loop
		 //start by checking if there is an innerfold
		var foldEndOffset=fold.end;
